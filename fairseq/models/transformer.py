...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        grid_img_features = self.grid_img_fc(grid_img_features)
    ...
    ################  txt to  image  #############
        ...
        # encoder_padding_mask = torch.cat([encoder_padding_mask, torch.zeros([grid_img_features.size(1), grid_img_features.size(0)]).type_as(encoder_padding_mask)], dim=1)

        region_img_mask = torch.sum(region_img_features,dim=-1).eq(0)
        grid_img_mask = torch.sum(grid_img_features,dim=-1).eq(1)

#        grid_img_mask = grid_img_mask.transpose(0,1)
	   #########   no grid features region mask  #############
#        region_img_mask = region_img_mask.transpose(0,1)
	   #########  full model mask  ##########
        region_img_mask = torch.cat([grid_img_mask,region_img_mask], dim=0).transpose(0,1)
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                x = layer(x, grid_img_features,region_img_features, encoder_padding_mask,region_img_mask, batch_len, idx)
